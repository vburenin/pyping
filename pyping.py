...
def ping(host, timeout=2, ping_id=None):
    """Ping remote host.
...
def ping_till_success(host, timeout=2, count=4):
    """Returns True if at least one ping attempt succeeded.
    """
    for _ in range(count):
        res = ping(host, timeout=timeout)
        if res is not None:
            return True
    return False
...
if __name__ == '__main__':
    print(ping_till_success('google.com'))
